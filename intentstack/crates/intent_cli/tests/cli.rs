...
#[test]
fn rust_cli_runs_core_plan_command() {
    let dir = env::temp_dir().join(format!("intentstack-core-plan-{}", std::process::id()));
    fs::create_dir_all(&dir).expect("create temp dir");
    let intent = dir.join("app.intent.yaml");
    fs::write(
        &intent,
...
"#,
    ...
    let out = Command::new(env!("CARGO_BIN_EXE_intentstack"))
        ...
        .output()
        .expect("run intentstack core plan");
    assert!(
        out.status.success(),
        "stderr: {}",
        String::from_utf8_lossy(&out.stderr)
    ...
    let stdout = String::from_utf8_lossy(&out.stdout);
    assert!(stdout.contains("\"emit_plan\""));
    assert!(stdout.contains("\"path\": \"lib/otel.ts\""));
    assert!(stdout.contains("\"path\": \"app/api/leads/route.ts\""));
    assert!(stdout.contains("\"path\": \"components/generated/LeadForm.tsx\""));
