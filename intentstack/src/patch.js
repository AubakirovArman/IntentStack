...
const OPS = {
  ...
  'content.block.add'(ast, op) {
    ...
    if (op.block?.id && section.blocks.some((b) => b.id === op.block.id)) throw new Error(`content block "${op.block.id}" already exists on ${op.section}`)
    ...
    section.blocks.splice(idx, 0, op.block)
