...
test('web_ts_minimal generates CRUD routes, API client, stats and pricing sections', async () => {
  ...
  assert.match(files['server/generated/db/client.ts'], /does not match manifest checksum/)
  ...
  assert.equal(JSON.parse(files['migrations/manifest.json']).migrations[0].id, '0000_init')
  assert.match(JSON.parse(files['migrations/manifest.json']).migrations[0].checksum, /^[a-f0-9]{64}$/)
...
test('next_shadcn generates CRUD routes, API client, stats and pricing sections', async () => {
  ...
  assert.match(files['lib/db/client.ts'], /does not match manifest checksum/)
