...
Both targets consume the same `db_driver` contract. Shipped drivers are `sqlite` and `postgres`;
the driver owns Drizzle imports, SQL migration text, migration manifest checksums, generated DB
client code, package dependencies, env examples, gitignore entries, and generated app README
database notes.
...
- Served editor mode supports drag/drop section reorder through generated `section.move` patches and
  a live page preview rendered directly from the current intent.
