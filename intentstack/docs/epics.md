...
7. CLI completeness: `done`. `new`, `check`, `build`, `apply`, `plan`, `diff`, `explain`, `doctor`, `migrate`, `list_capabilities`, `schema`, `graph`, `openapi`, `testgen`, `deploy`, `stats`, `verify`, and `docs` exist; `build` now includes normalize, format, and generated build verification phases.
