      ...
      <div className="max-w-6xl mx-auto px-4 grid gap-10 lg:grid-cols-[220px_minmax(0,1fr)]">
        <aside className="hidden lg:block">
          <nav className="sticky top-24 rounded-lg border border-base-200 bg-base-100 p-4 text-sm">
            ...
            <a className="ml-3 block py-1 opacity-70 hover:opacity-100" href="#cards-example">{"Card grid"}</a>
            <a className="ml-3 block py-1 opacity-70 hover:opacity-100" href="#form-example">{"Form"}</a>
            <a className="ml-3 block py-1 opacity-70 hover:opacity-100" href="#table-example">{"Table"}</a>
            <a className="ml-3 block py-1 opacity-70 hover:opacity-100" href="#pricing-example">{"Pricing cards"}</a>
            <a className="ml-3 block py-1 opacity-70 hover:opacity-100" href="#stats-example">{"Stats"}</a>
        ...
        <article className="space-y-6">
          ...
          <p className="text-base leading-7 opacity-80">{"Each example below is paired: the generated live component appears first, and the patch code that creates the same kind of component appears directly below it."}</p>
          ...
          <div className="rounded-lg border border-info/30 bg-info/10 p-4">
            ...
            <p className="leading-7 opacity-80">{"Read each example as a pattern: inspect the generated component, then copy the patch below it and adapt the ids, text, fields, or records for your app."}</p>
          </div>
          <div id="cards-example" className="rounded-lg overflow-hidden border border-base-200 bg-base-100 shadow-sm">
            <div className="space-y-2 border-b border-base-200 p-4">
            <h3 className="text-xl font-semibold">{"Card grid"}</h3>
            <p className="text-base leading-7 opacity-80">{"A card_grid section is useful for feature cards, service lists, galleries, and compact repeated content. The preview is generated from the section module; the patch below shows how to create the same kind of module."}</p>
            </div>
            ...
            <div className="border-t border-base-200 bg-base-200 p-4">
              ...
              <pre className="overflow-x-auto text-sm"><code>{"version: 0.1\npatch:\n  - op: section.module.add\n    page: docs\n    after: docs_content\n    section:\n      id: docs_component_cards\n      type: card_grid\n      title: Live card_grid example\n      embed_only: true\n      columns: 3\n      items:\n        - title: Card title\n          text: A card_grid section renders repeated cards from structured items.\n        - title: Agent-friendly\n          text: Each card is data, so an agent can add, remove, or update cards safely.\n        - title: Target-owned UI\n          text: The intent declares the content; the target adapter owns the React markup.\n"}</code></pre>
            </div>
          </div>
          <div id="form-example" className="rounded-lg overflow-hidden border border-base-200 bg-base-100 shadow-sm">
            <div className="space-y-2 border-b border-base-200 p-4">
            <h3 className="text-xl font-semibold">{"Form"}</h3>
            <p className="text-base leading-7 opacity-80">{"A form usually needs an entity and a create_record action. The compiler then generates the UI, request handler, validation schema, database insert, and API client."}</p>
            </div>
            <div className="bg-base-200/40 [&_section]:py-8 [&_section_.max-w-6xl]:max-w-none">
              <DocsComponentForm />
            </div>
          ...
          <div id="table-example" className="rounded-lg overflow-hidden border border-base-200 bg-base-100 shadow-sm">
            <div className="space-y-2 border-b border-base-200 p-4">
            <h3 className="text-xl font-semibold">{"Table"}</h3>
            <p className="text-base leading-7 opacity-80">{"A table connects to a list_records action. The generated app owns loading, empty, and error states while the intent declares the entity, source action, and visible columns."}</p>
            </div>
            <div className="bg-base-200/40 [&_section]:py-8 [&_section_.max-w-6xl]:max-w-none">
              <DocsComponentTable />
            </div>
          ...
          <div id="pricing-example" className="rounded-lg overflow-hidden border border-base-200 bg-base-100 shadow-sm">
            <div className="space-y-2 border-b border-base-200 p-4">
            <h3 className="text-xl font-semibold">{"Pricing cards"}</h3>
            <p className="text-base leading-7 opacity-80">{"pricing_cards is a structured marketing section. Plans, prices, and features stay editable as data while each target controls presentation."}</p>
            </div>
            <div className="bg-base-200/40 [&_section]:py-8 [&_section_.max-w-6xl]:max-w-none">
          ...
          <div id="stats-example" className="rounded-lg overflow-hidden border border-base-200 bg-base-100 shadow-sm">
            <div className="space-y-2 border-b border-base-200 p-4">
            <h3 className="text-xl font-semibold">{"Stats"}</h3>
            <p className="text-base leading-7 opacity-80">{"stats works well for product metrics, delivery promises, usage numbers, and operational summaries. Keep each stat as data so future patches can update one value safely."}</p>
            </div>
            <div className="bg-base-200/40 [&_section]:py-8 [&_section_.max-w-6xl]:max-w-none">
              <DocsComponentStats />
            </div>
